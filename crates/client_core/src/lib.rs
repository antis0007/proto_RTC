    ...
    pub async fn upload_file(
        ...
        let (server_url, user_id) = {
            ...
            (
                ...
                guard.user_id.ok_or_else(|| anyhow!("not logged in"))?,
            )
        };
        ...
        let url = format!("{server_url}/files/upload");
        let mut req = self.http.post(url).query(&query).body(bytes);
        if let Some(content_type) = content_type {
            req = req.header(reqwest::header::CONTENT_TYPE, content_type);
        }

        let res = req.send().await?.error_for_status()?;
        let body: FileUploadResponse = res.json().await?;
